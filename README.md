...
当前版本：v0.3.2。判断区、标记区、字段位置和样式会按模板内容自动保存在本机；重新选择同一模板或重启程序后会自动恢复。
...
- 在打印预览中分别调整楼栋、房号、完整地址、姓名、电话的字体、字号、粗体和位置；可直接拖动文字，也可用方向键微调（普通 1pt、Shift 5pt、Ctrl 0.1pt），停手后自动校准为精确的 Word→PDF 效果。
- 自动把 `1-101`、`1栋101室` 等地址拆成楼栋与房号，并保留模板原有的“栋/幢/室”和下划线格式。
- 支持一人一份 DOCX 或合并为单个 DOCX。
- 支持纯净打印模式、异常数据检查和 XLSX 汇总。
- 内置“检查更新”按钮，从 GitHub Releases 检查正式版本。
...
4. 已安装的程序点击“检查更新”后，会匿名访问公开的 GitHub Releases，比对版本并打开新版下载页。
